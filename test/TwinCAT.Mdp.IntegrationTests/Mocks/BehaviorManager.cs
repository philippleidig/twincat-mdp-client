...
namespace TwinCAT.Mdp.IntegrationTests.Mocks
{
	public class BehaviorManager
	{
		...
		private Dictionary<Type, List<Behavior>> _BehaviorDictionary =
			new Dictionary<Type, List<Behavior>>();
		...
		public IEnumerable<T>? GetBehaviorOfType<T>()
			where T : Behavior
		{
			return _BehaviorDictionary
				.Where(k => k.Key == typeof(T))
				.Select(v => v.Value)
				.FirstOrDefault()
				?.Cast<T>();
		}

		public T? GetBehaviorOfType<T>(Func<T, bool> matchFilter)
			where T : Behavior
		{
			return GetBehaviorOfType<T>()?.Where(b => matchFilter(b)).FirstOrDefault();
		}
	}
}
