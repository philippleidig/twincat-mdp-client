...
namespace TwinCAT.Mdp.IntegrationTests.Mocks
{
	internal class DeviceManagerMock : AdsServerMock, IDisposable
	{
		...
		private readonly Dictionary<uint, DeviceManagerModuleMock> _modules =
			new Dictionary<uint, DeviceManagerModuleMock>();

		private readonly Dictionary<uint, byte[]> _writeableMdpAddresses =
		...
		private void RegisterModules()
		{
			// CPU
			RegisterModule(
				new DeviceManagerModuleMock(1, ModuleType.CPU).RegisterMdpAddress<uint>(
					1,
					1,
					BitConverter.GetBytes(2496)
				)
			);

			// NIC
			RegisterModule(
				new DeviceManagerModuleMock(2, ModuleType.NIC)
					// NIC Name
					.RegisterMdpAddress<string>(0, 3, Encoding.ASCII.GetBytes("em0"))
					// NIC DHCP
					.RegisterMdpAddress<bool>(1, 4, BitConverter.GetBytes(true), false)
					// Test parameter for write tests (supports various data types)
					.RegisterMdpAddress<long>(1, 5, BitConverter.GetBytes(0L), false)
			);

			// Time
			RegisterModule(
				new DeviceManagerModuleMock(3, ModuleType.Time)
					// SNTP Server address
					.RegisterMdpAddress<string>(
						1,
						1,
						Encoding.ASCII.GetBytes("111.111.111.111"),
						false
					)
		...
		private void OnWriteIndication(
			uint indexGroup,
			uint indexOffset,
