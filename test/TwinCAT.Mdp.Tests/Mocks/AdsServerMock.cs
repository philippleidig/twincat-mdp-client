...
namespace TwinCAT.Mdp.Tests.Mocks
{
	...
	/// </summary>    
	public record ReadIndicationBehavior(uint IndexGroup, uint IndexOffset, Memory<byte> ResponseData, AdsErrorCode ErrorCode = AdsErrorCode.Succeeded, Func<uint, uint, ReadOnlyMemory<byte>> onCall = null)
		: Behavior(IndexGroup, IndexOffset, ResponseData, ErrorCode);

	/// <summary>
	/// Behavior / response for a ADS WriteIndication
	/// </summary>    
	public record WriteIndicationBehavior(uint IndexGroup, uint IndexOffset, int ExpectedLength, AdsErrorCode ErrorCode = AdsErrorCode.Succeeded, Action<uint, uint, ReadOnlyMemory<byte>> onCall = null)
	...
	public class AdsServerMock : AdsServer
    {
        ...
        public AdsServerMock(ushort port, string portName) : base(port, portName)
        {
            Init(portName);
            Connect();
        }
        ...
        protected override Task<ResultReadBytes> OnReadAsync(AmsAddress sender, uint invokeId, uint indexGroup, uint indexOffset, int readLength, CancellationToken cancel)
        {
			...
			if(behavior.onCall != null)
			{
				...
				{
					return Task.FromResult(new ResultReadBytes(behavior.ErrorCode, responseData.Slice(0, len)));
				}

				return Task.FromResult(new ResultReadBytes(behavior.ErrorCode, behavior.ResponseData.Slice(0, len)));
			}
